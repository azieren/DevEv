...
def create_semi_sphere(radius, num_segments, color=(0.5,0.5,0.5, 0.4), edgecolor=(1.0,1.0,0.0, 1.0)):
    phi = np.linspace(0, np.pi, num_segments)  # Limit phi to 90 degrees
    ...
    for i in range(num_segments - 1):
        for j in range(num_segments - 1):
            p1 = i * num_segments + j
            p2 = (i + 1) * num_segments + j
            p3 = i * num_segments + (j + 1)
            p4 = (i + 1) * num_segments + (j + 1)
    ...
    md = MeshData()
    md.setVertexes(np.vstack([x.flatten(), y.flatten(), z.flatten()]).T)
    md.setFaces(np.array(faces))

    
    sphere = GLMeshSphereItem(meshdata=md, smooth=True, color=color, shader='balloon', 
        ...
        if self.opts['drawEdges']:
            ...
            try:
                ...
                if self.edgeColors is None:
                    ...
                    glEnableClientState(GL_COLOR_ARRAY)
                    glColorPointerf(self.edgeColors)
                glLineWidth(2.0)
                edges = edges.flatten()
